...
export default {
        ...
        limit: {
          buy: {
            ...
            total: {
              ...
              minValue: minValue(0),
              maxValue: maxValue(
                this.wallet.source !== undefined
                  ? this.wallet.source.balance
                  : 0
              ),
            },
          },
          sell: {
            amount: {
              ...
              minValue: minValue(0),
              maxValue: maxValue(
                this.wallet.coin !== undefined ? this.wallet.coin.balance : 0
              ),
            },
            total: {
              required,
  ...
  watch: {
    ...
    "form.limit.buy.price"(value) {
      if (
        !(
          this.form.limit.buy.amount === null ||
          this.form.limit.buy.total === null
        )
      ) {
        this.form.limit.buy.total = isNaN(value * this.form.limit.buy.amount)
          ? 0
          : value * this.form.limit.buy.amount;
      }
    },
    "form.limit.buy.amount"(value) {
      if (this.form.limit.buy.price !== null) {
        this.form.limit.buy.total = isNaN(this.form.limit.buy.price * value)
          ? 0
          : this.form.limit.buy.price * value;
      } else {
        this.form.limit.buy.total = isNaN(this.marketStatus.sell_price * value)
          ? 0
          : this.marketStatus.sell_price * value;
      }
    },
    "form.limit.buy.total"(value) {
      if (this.form.limit.buy.price !== null) {
        this.form.limit.buy.amount = isNaN(value / this.form.limit.buy.price)
          ? 0
          : value / this.form.limit.buy.price;
      } else {
        if (this.marketStatus.sell_price != 0) {
          this.form.limit.buy.amount = isNaN(
            value /
              (this.marketStatus.sell_price == 0
                ? 1
                : this.marketStatus.sell_price)
          )
            ? 0
            : value /
              (this.marketStatus.sell_price == 0
    ...
    "form.limit.buy.percent"(value) {
      this.form.limit.buy.total = isNaN(
        (this.wallet.source !== undefined ? this.wallet.source.balance : 0) /
          value
      )
        ? 0
        : (this.wallet.source !== undefined ? this.wallet.source.balance : 0) /
    ...
    "form.limit.sell.price"(value) {
      if (
        !(
          this.form.limit.sell.amount === null ||
          this.form.limit.sell.total === null
        )
      ) {
        this.form.limit.sell.total = isNaN(value * this.form.limit.sell.amount)
          ? 0
          : value * this.form.limit.sell.amount;
      }
    },
    "form.limit.sell.amount"(value) {
      if (this.form.limit.sell.price !== null) {
        this.form.limit.sell.total = isNaN(this.form.limit.sell.price * value)
          ? 0
          : this.form.limit.sell.price * value;
      } else {
        this.form.limit.sell.total = isNaN(this.marketStatus.buy_price * value)
          ? 0
          : this.marketStatus.buy_price * value;
      }
    },
    "form.limit.sell.total"(value) {
      if (this.form.limit.sell.price !== null) {
        this.form.limit.sell.amount = isNaN(value / this.form.limit.sell.price)
          ? 0
          : value / this.form.limit.sell.price;
      }
    },
    "form.limit.sell.percent"(value) {
      this.form.limit.sell.amount = isNaN(
        ((this.wallet.coin !== undefined ? this.wallet.coin.balance : 0) /
          100) *
          value
      )
    ...
    "form.market.buy.amount"(value) {
      this.form.market.buy.total = isNaN(
        value *
          (this.marketStatus.sell_price === 0
            ? 1
            : this.marketStatus.sell_price)
    ...
    "form.market.buy.total"(value) {
      this.form.market.buy.amount = isNaN(
        value /
          (this.marketStatus.sell_price === 0
            ? 1
            : this.marketStatus.sell_price)
    ...
    "form.market.sell.amount"(value) {
      this.form.market.sell.total = isNaN(
        value *
          (this.marketStatus.buy_price === 0 ? 1 : this.marketStatus.buy_price)
      )
        ? 0
        : value *
          (this.marketStatus.buy_price === 0 ? 1 : this.marketStatus.buy_price);
    ...
    "form.market.sell.total"(value) {
      this.form.market.sell.amount = isNaN(
        value /
          (this.marketStatus.buy_price === 0 ? 1 : this.marketStatus.buy_price)
      )
        ? 0
