<template>
    ...
    >
        <b-card-text>
            <b-row>
                <b-col>
                    <b-form-input v-model="search" class="search" :placeholder="$t('Parite Çifti Arayın')"></b-form-input>
                </b-col>
            </b-row>
            <TabView v-model:activeIndex="tabindex">
                <TabPanel>
                    ...
                    <table class="table table-sm order-table">
                        ...
                        <tbody>
                        <tr
                            v-for="(coin,coin_key) in Object.values(coins).filter(element => element.user_favorite===true)"
                            :class="(selectedCoin.coin.symbol===coin.coin.symbol && selectedCoin.source.symbol===coin.source.symbol?'selected':'')" style="cursor: pointer;"
                            :key="coin_key"
                        ...
                        >
                            ...
                            <td :class="coin.parity_price.price.status">{{ coin.parity_price.price.value }}</td>
                            <td v-if="coin.parity_price.percent_last_24_hours!==undefined" :class="coin.parity_price.price.status">{{ parseFloat(coin.parity_price.percent_last_24_hours.value).toFixed(2) }}%</td>
                            <td v-else>-%</td>
                        </tr>
                        </tbody>
                    </table>
                ...
                >
                    <table class="table table-sm order-table" :key="search">
                        ...
                        <tr
                            :class="(selectedCoin.coin.symbol===coin.coin.symbol && selectedCoin.source.symbol===coin.source.symbol?'selected':'')" style="cursor: pointer;"
                            v-for="(coin,coin_key) in coin_tables.filter(element => (this.search==='' || element.coin.symbol.indexOf(this.search)>=0))"
                            :key="coin_key"
                            @click="selectCoin(coin)"
                        >
                            <td @click="addFavorite({coin:coin.coin.symbol, source:coin.source.symbol})">
...
export default {
    ...
    methods: {
        selectCoin(value) {
            ...
            this.$router.push({name: 'exchange', params: {parity: value.source.symbol + '-' + value.coin.symbol}})
        },
        async addFavorite(parity) {
            await restAPI.getData({
                Action: "exchange/favorite",
            }, parity).then((response) => {
