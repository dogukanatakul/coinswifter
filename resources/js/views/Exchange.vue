  ...
  <b-container class="exchange pb-5 mb-3" fluid>
    <b-row class="d-none d-lg-flex">
      <b-col cols="12" xs="3" sm="12" lg="3" class="d-sm-block">
        <MarketPairs v-if="Object.values(selectedCoin).length > 0" :key="selectedCoin" v-model:selectedCoin="selectedCoin" v-model:parities="parities" v-model:coins="coins" @getTokens="getTokens" />
      </b-col>
      <b-col cols="12" lg="6">
        <b-row>
      ...
      <b-col cols="12" lg="3">
        <wallet v-if=" Object.values(selectedCoin).length > 0 && Object.values(wallet).length > 0 " :wallet.sync="wallet" :key="wallet" class="d-none d-lg-block" @sendTradeFormWallet="sendTradeFormWallet" ></wallet>
...
export default {
  ...
  async created() {
    // this.loader = await this.$loading.show({ container: null, canCancel: false, onCancel: this.onCancel,
    // });
  ...
  methods: {
    async getParity(loader = false) {
      // console.log(selectedCoinValue); console.log(parts); Source = TRY Coin = CATE
...
@media screen and (max-width: 1690px) and(min-width: 992px) {
  // .title, .price, .info { font-size: 8px !important; // text-align: center;
  // }
  .d-xs-flex {
